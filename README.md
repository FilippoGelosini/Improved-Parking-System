...
By default, `runner.py` loads the scenario that "matches" the total population number specified in `TOTAL_POPULATION`, e.g. `TOTAL_POPULATION = 80` loads `scenarios/park_80.sumocfg`. Can be edited if needed. I'll probably move the string containing the path to the scenario to `constants.py`